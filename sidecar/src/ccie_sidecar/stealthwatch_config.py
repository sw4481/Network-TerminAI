"""Read Stealthwatch connection details from the app config DB.

Mirrors ccie_sidecar.proxmox_api.config: the connection lives in the same
...
def _from_env() -> dict[str, Any] | None:
    ...
    return {
        ...
        "verify_ssl": os.environ.get("STEALTHWATCH_VERIFY_SSL", "1").lower()
        in ("1", "true", "yes"),
    }


def get_stealthwatch_config() -> dict[str, Any] | None:
    """Return the saved Stealthwatch connection dict, or None if unset.

    Reads sessions.db.stealthwatch_config first; falls back to STEALTHWATCH_*
    env vars (which is how the MCP-server transport delivers them).
    """
    db = _db_path()
    try:
        if db.exists():
            ...
            try:
                ...
                cur.execute(
                    ...
                    "FROM stealthwatch_config WHERE id = 1"
                )
                row = cur.fetchone()
            finally:
                conn.close()
