"""Read Cisco ISE connection details from the app config DB.

Mirrors ccie_sidecar.stealthwatch_config: the connection lives in the same
...
def _from_env() -> dict[str, Any] | None:
    ...
    return {
        ...
        "verify_ssl": os.environ.get("ISE_VERIFY_SSL", "1").lower()
        in ("1", "true", "yes"),
    }


def get_ise_config() -> dict[str, Any] | None:
    """Return the saved Cisco ISE connection dict, or None if unset.

    Reads sessions.db.ise_config first; falls back to ISE_* env vars (which is
