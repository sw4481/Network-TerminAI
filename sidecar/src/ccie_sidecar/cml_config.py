"""Read Cisco Modeling Labs connection details from the app config DB.

Mirrors ccie_sidecar.ise_config: the connection lives in the same
...
def _from_env() -> dict[str, Any] | None:
    ...
    return {
        ...
        "verify_ssl": os.environ.get("CML_VERIFY_SSL", "0").lower()
        in ("1", "true", "yes"),
    }


def get_cml_config() -> dict[str, Any] | None:
    """Return the saved Cisco Modeling Labs connection dict, or None if unset.

    Reads sessions.db.cml_config first; falls back to CML_* env vars (which is
