"""Read Cisco ACI (APIC) connection details from the app config DB.

Mirrors ccie_sidecar.cml_config: the connection lives in the same sessions.db
...
def _from_env() -> dict[str, Any] | None:
    ...
    return {
        ...
        "verify_ssl": os.environ.get("ACI_VERIFY_SSL", "0").lower()
        in ("1", "true", "yes"),
    }


def get_aci_config() -> dict[str, Any] | None:
    """Return the saved Cisco ACI connection dict, or None if unset.

    Reads sessions.db.aci_config first; falls back to ACI_* env vars (which is
